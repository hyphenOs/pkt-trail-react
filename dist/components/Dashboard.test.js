...
describe("<Dashboard packets={packets} config={config}/>", () => {
  ...
  it("Does not mount Table component if packets and config are undefined", () => {
    const wrapper = shallow( /*#__PURE__*/React.createElement(Dashboard, null)); // Equivalent to <Dashboard packets={undefined} config={undefined} />

    expect(wrapper.exists("Table")).toEqual(false);
  });
  it("Mounts Table component if packets and config are defined", () => {
    const wrapper = shallow( /*#__PURE__*/React.createElement(Dashboard, {
      packets: [null],
  ...
  it("Uses defaultConfig when no config is provided", () => {
    ...
    expect(wrapper.find("Table").props().config).toEqual(defaultConfig.tableConfig);
  });
  /**
  ...
  it("Merges passed config with defaultConfig on initial render", () => {
    ...
    expect(Table.props().config).toEqual({ ...defaultConfig.tableConfig,
      ...config.tableConfig
    });
  });
});